...
def get_encoding(page):
    ...
    print "Document has the following encodings: %s" % declared_encodings
    ...
    for declared_encoding in declared_encodings:
        ...
        try:
            ...
            print 'Using encoding "%s"' % encoding
            return encoding
        except UnicodeDecodeError:
            print 'Encoding "%s", specified in the document as "%s" didn\'t work' % (encoding, declared_encoding)

    # Fallback to chardet if declared encodings fail
    text = re.sub('</?[^>]*>\s*', ' ', page)
    enc = 'utf-8'
    if not text.strip() or len(text) < 10:
        print "Can't guess encoding because text is too short"
        return enc
    res = chardet.detect(text)
    enc = fix_charset(res['encoding'])
    print 'Trying encoding "%s" guessed with confidence %.2f' % (enc, res['confidence'])
